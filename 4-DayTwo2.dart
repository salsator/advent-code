void main() {
  ...
  for (String range in ranges) {
    ...
    for (int i = start; i <= end; i++) {
      ...
      for (int j = 1; j <= charCount ~/ 2; j++) {
        String number = count.substring(0, j);

        if (charCount % j != 0) {
          continue;
        }
        int repeat = charCount ~/ j;

        String repeated = List.filled(repeat, number).join();
